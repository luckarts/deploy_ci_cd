...
| 8 | `loki`: `mkdir /data/loki/chunks: permission denied` | L'image `grafana/loki` tourne en UID `10001` (non-root), le volume nommé est créé `root` par défaut | Service `loki-init` (alpine, `chown -R 10001:10001`) avant `loki`, via `depends_on: condition: service_completed_successfully` | `b751aee` |
